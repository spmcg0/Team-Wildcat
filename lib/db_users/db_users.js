...
function getUserTweets(user, callback){
	db.all("SELECT t.tweet as tweet, t.imageLoc as image_loc, t.uname as uname, t.first_name as name FROM Tweets t WHERE t.email='" + user.email + "'", function(err, row){
		if (row === undefined){
...
function getUserFollowers(user, callback){
	db.all("SELECT * from Users u, Followers f WHERE f.followee='" + user.email + "' AND u.email=f.follower", function(err, row){
		console.log(row);
		if (row===undefined){
...
function getUserFollowings(user, callback){
	db.all("SELECT * from Users u, Followers f WHERE f.follower='" + user.email + "' AND u.email=f.followee", function(err, row){
		console.log(row);
		if (row===undefined){
