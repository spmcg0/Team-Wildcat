...
function addUserTweet(tweetData, callback) {
	...
	for (var j = 0; j < tweetLen; j++){
		...
		if (userTweets.uname === tweetData.uname) {
			...
			console.log(userTweets.uname);
			console.log(userTweets.tweets.name);
			console.log(userTweets.tweets.image_loc);
			console.log(userTweets.tweets.tweet);
			returned = true;
			userTweets.tweets.push(tweetData.tweets);
			callback(undefined, userTweets.tweets);
		}
	}
	console.log(tweetdb.length);
	if(!returned){
		console.log("here");
		returned = true;
		tweetdb.push(tweetData);
		callback(undefined, tweetData.tweets);
		var userTweets = tweetdb[1];
		console.log(userTweets.uname);
		console.log(userTweets.tweets.name);
		console.log(userTweets.tweets.image_loc);
		console.log(userTweets.tweets.tweet);
